<template>
  <el-card class="app-card" shadow="hover" :class="{ 'detail-card': isDetail }">
    ...
    <div class="app-card-content">
      ...
      <div class="app-banner" v-if="bgUrl">
        <img :src="bgUrl" class="app-banner-img" alt="banner" />
      </div>
      <div class="app-header">
        <img v-if="iconUrl" :src="iconUrl" class="app-icon" alt="icon" />
...
const iconUrl = computed(() => {
  ...
  const link = item.url || item.link || item.homepage;
  if (!link) return null;
  try {
...
const bgUrl = computed(() => {
  const item = props.item;
  return item.bg_url || item.banner || item.cover || item.image || item.thumbnail || item.screenshot || null;
});
